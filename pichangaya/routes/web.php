...
Route::get('/', function (Request $request) {
    ...
    // --- OBTENCIÓN DE DATOS ---
    // Usamos 'with' para optimizar y evitar errores si falta info en la BD
    $canchas = $query->with(['district', 'sport', 'images'])->get();
    
    // Listas para los selectores del buscador
...
// RUTA PÚBLICA: Detalle de Cancha (Accesible para todos)
...
Route::middleware(['auth:sanctum', config('jetstream.auth_session'), 'verified'])->group(function () {
    ...
    // Dashboard (Redundante si usas Home, pero útil si quieres un panel privado)
    Route::get('/dashboard', [DashboardController::class, 'index'])->name('dashboard');

    // --- RUTAS DE RESERVAS DEL CLIENTE ---
    
    // 🟢 [IMPORTANTE] Muestra el formulario para crear una reserva nueva
...
// REDIRECCIÓN INTELIGENTE (Opcional, útil para redirigir tras login)
