        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100">
                ...
                <div class="bg-amber-50 px-6 py-4 border-b border-amber-100 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-medium text-amber-900">Actualizar Información</h3>
                        <p class="text-sm text-amber-700">Modifica los detalles de la cancha.</p>
                    </div>
                </div>
                    ...
                    <form action="{{ route('admin.canchas.update', $cancha) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div class="col-span-1 md:col-span-2">
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $cancha->name) }}" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 transition-all py-3" required>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" step="0.01" name="price_per_hour" id="price_per_hour" value="{{ old('price_per_hour', $cancha->price_per_hour) }}" class="pl-8 w-full rounded-xl border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 py-3 font-semibold text-gray-700" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-8">
                            ...
                            <p class="text-sm text-gray-500 mb-4">Selecciona máximo 2.</p>
                        ...
                        <div class="mb-8 space-y-4">
                            ...
                            <div class="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                                <label class="block text-lg font-bold text-gray-700 mb-3">Imágenes Actuales</label>
                                    ...
                                    @forelse($cancha->getMedia('canchas') as $media)
                                        <div class="relative group rounded-xl overflow-hidden shadow-sm border border-gray-200">
                                            ...
                                            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                <label class="cursor-pointer bg-red-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg hover:bg-red-700 transform hover:scale-105 transition-all">
                                                    <input type="checkbox" name="delete_images[]" value="{{ $media->id }}" class="hidden">
                                                    Eliminar
                                                </label>
                                            </div>
                                            {{-- Indicador visual si se marca para eliminar (requeriría JS extra, por ahora el botón es claro) --}}
                                        </div>
                                    @empty
                                        <p class="text-sm text-gray-500 col-span-full italic">No hay imágenes cargadas.</p>
                                    @endforelse
                                </div>
                                <p class="text-xs text-gray-500 mt-2">* Marca "Eliminar" y guarda los cambios para borrar la foto.</p>
                            </div>

                            {{-- Subir Nuevas --}}
                            <div class="border-2 border-dashed border-amber-300 rounded-2xl p-6 flex flex-col items-center justify-center bg-amber-50/50 hover:bg-amber-50 transition-colors">
                                <label for="images" class="cursor-pointer text-center">
                                    <div class="mx-auto bg-amber-100 p-3 rounded-full w-fit mb-2">
                                        <svg class="w-6 h-6 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                                    </div>
                                    <span class="block text-sm font-bold text-amber-700">Agregar Nuevas Fotos</span>
                                    <input type="file" name="images[]" id="images" multiple accept="image/*" class="hidden">
                                </label>
                                <div id="image-preview" class="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 w-full"></div>
                            </div>
                        </div>
                        ...
                        <div class="mb-8">
                            ...
                            <textarea name="description" id="description" rows="3" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500">{{ old('description', $cancha->description) }}</textarea>
                        </div>
                        ...
                        <div class="flex justify-end gap-4 border-t border-gray-100 pt-6">
                            ...
                            <button type="submit" class="bg-amber-500 hover:bg-amber-600 text-white font-bold py-2.5 px-6 rounded-xl shadow-lg shadow-amber-500/30 hover:-translate-y-0.5 transition-all">
                                Guardar Cambios
                            </button>
                        </div>
                    </form>
...
<script>
    // --- LÓGICA DE MÁXIMO 2 DEPORTES (Igual que Create) ---
        ...
        for (const file of event.target.files) {
            if (file.type.startsWith('image/')) {
                ...
                reader.onload = (e) => {
                    ...
                    img.className = 'w-full h-20 object-cover rounded-lg shadow-sm border border-amber-200';
