<x-app-layout>
    ...
    <x-slot name="header">
        <header class="flex items-center gap-3">
            <figure class="bg-indigo-600 p-2 rounded-lg text-white">
                    ...
                    <form action="{{ route('admin.owners.canchas.store', $owner) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <section class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <span class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-500 sm:text-sm">S/</span>
                                ...
                                @foreach($sports as $sport)
                                    <label class="cursor-pointer group relative">
                                        ...
                                        <section class="p-4 bg-white border-2 border-gray-200 rounded-xl hover:border-indigo-300 hover:shadow-lg transition-all duration-200 peer-checked:border-indigo-600 peer-checked:bg-indigo-50 peer-checked:text-indigo-700 flex flex-col items-center justify-center gap-3 h-full peer-disabled:opacity-50 peer-disabled:cursor-not-allowed">
                                            <figure class="p-3 rounded-full bg-gray-100 text-gray-500 group-hover:scale-110 group-hover:bg-indigo-100 group-hover:text-indigo-600 peer-checked:bg-indigo-600 peer-checked:text-white transition-all duration-300">
                                            ...
                                            <figure class="absolute top-2 right-2 opacity-0 peer-checked:opacity-100 text-indigo-600 transition-opacity">
                        ...
                        {{-- SECCIÓN 3: SERVICIOS --}}
                        <fieldset class="mb-8">
                            <legend class="block text-lg font-bold text-gray-800 mb-1">Servicios Adicionales</legend>
                                ...
                                @isset($services)
                                    @foreach($services as $service)
                                        <label class="cursor-pointer group relative">
                                            ...
                                            >
                                            <section class="px-4 py-3 bg-white border border-gray-200 rounded-lg hover:bg-emerald-50 hover:border-emerald-300 transition-all duration-200 peer-checked:border-emerald-500 peer-checked:bg-emerald-50 peer-checked:text-emerald-700 flex items-center gap-3 shadow-sm peer-checked:shadow-md">
                                                <figure class="text-gray-400 peer-checked:text-emerald-600">
                        ...
                        <section class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                            <div>
                                <label for="open_time" class="block text-sm font-bold text-gray-700 mb-1">Hora Apertura</label>
                                <input type="time" name="open_time" id="open_time" value="{{ old('open_time', '08:00') }}" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 py-2.5" required>
                            </div>
                            <div>
                                <label for="close_time" class="block text-sm font-bold text-gray-700 mb-1">Hora Cierre</label>
                                <input type="time" name="close_time" id="close_time" value="{{ old('close_time', '23:00') }}" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 py-2.5" required>
                            </div>
                            <div>
                                <label for="contact_phone" class="block text-sm font-bold text-gray-700 mb-1">WhatsApp de Contacto</label>
                                <input type="text" name="contact_phone" id="contact_phone" value="{{ old('contact_phone', $owner->phone) }}" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 py-2.5" required>
                            </div>
                        </section>

                        {{-- SECCIÓN 5: UBICACIÓN Y MAPA --}}
                        <section class="mb-8">
                            <h4 class="text-lg font-bold text-gray-800 mb-2">Ubicación Exacta</h4>
                            <div class="mb-4">
                            ...
                            <div class="relative w-full h-96 rounded-2xl overflow-hidden shadow-lg border-2 border-white ring-1 ring-gray-200">
                                ...
                                <aside class="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-md shadow text-xs font-bold text-gray-600 flex items-center gap-1">
                        ...
                        {{-- SECCIÓN 6: IMÁGENES --}}
                        <section class="mb-8 bg-indigo-50 rounded-2xl p-6 border border-indigo-100">
                        ...
                        {{-- Botones de Acción --}}
                        <footer class="flex items-center justify-end gap-4 mt-8 pt-6 border-t border-gray-100">
    ...
    {{-- SCRIPTS ORIGINALES PRESERVADOS --}}
