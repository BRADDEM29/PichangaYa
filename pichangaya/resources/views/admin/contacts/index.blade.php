                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th class="px-6 py-3">Celular</th> {{-- 🟢 COLUMNA NUEVA --}}
                        ...
                        <tbody>
                            @forelse ($contacts as $contact)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs font-semibold mt-1 bg-gray-100 dark:bg-gray-900 inline-block px-1 rounded">{{ $contact->subject }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end gap-2">
                                            <button onclick="alert('Mensaje completo:\n\n{{ addslashes($contact->message) }}')" 
                                                    class="text-blue-600 hover:text-blue-900 font-bold text-xs uppercase">Ver</button>
                                            
                                            <form action="{{ route('admin.contacts.destroy', $contact->id) }}" method="POST" onsubmit="return confirm('¿Eliminar?')">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-900 font-bold text-xs uppercase">Borrar</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-10 text-center text-gray-500 italic">Buzón vacío.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
