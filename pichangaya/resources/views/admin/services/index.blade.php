        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-white shadow-xl sm:rounded-lg p-6 h-fit">
                    <h3 class="text-lg font-bold mb-4">Agregar Nuevo Servicio</h3>
                    <form action="{{ route('admin.services.store') }}" method="POST">
                        @csrf
                        <div>
                            <label class="block font-medium text-sm text-gray-700">Nombre</label>
                            <input type="text" name="name" class="border-gray-300 rounded-md shadow-sm block w-full mt-1" placeholder="Ej: Wi-Fi" required>
                        </div>
                        <div class="mt-4">
                                ...
                                <tr>
                                    ...
                                    <form action="{{ route('admin.services.update', $service->id) }}" method="POST">
                                        ...
                                        <td class="px-6 py-4 w-16">
                                            <input type="text" name="icon" value="{{ $service->icon }}" class="text-center text-xl border-gray-300 rounded-md shadow-sm w-12 p-1">
                                        </td>
                                        <td class="px-6 py-4">
                                            <input type="text" name="name" value="{{ $service->name }}" class="text-sm border-gray-300 rounded-md shadow-sm w-full p-1">
                                        </td>
