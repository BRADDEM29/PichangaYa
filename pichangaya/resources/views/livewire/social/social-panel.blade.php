...
>
    ...
    {{-- 🔴 PANEL PRINCIPAL --}}
    @if($isOpen)
        <div class="bg-gray-900 border border-gray-700 rounded-lg shadow-2xl w-80 overflow-hidden mb-2 flex flex-col h-[500px]">
            
            {{-- TABS SUPERIORES (Estilo Dota/Esports) --}}
            @if(!$activeChatUser)
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
                ...
                @if($activeTab === 'friends')
                    ...
                    @if($activeChatUser)
                        ...
                        <div x-ref="chatContainer" wire:poll.3s="loadMessages" class="flex-1 overflow-y-auto p-3 space-y-3 bg-gray-950/50">
                            ...
                            @empty
                                <div class="text-center py-10"><p class="text-xs text-gray-500">¡Saluda a {{ $activeChatUser->name }}!</p></div>
                            @endforelse
                        </div>
                        ...
                        <form wire:submit.prevent="sendMessage" class="p-2 border-t border-gray-700 bg-gray-900">
                            <div class="flex gap-2">
                                <input type="text" wire:model="newMessage" class="flex-1 bg-gray-800 border-none rounded-full px-4 py-2 text-xs text-white focus:ring-1 focus:ring-blue-500" placeholder="Escribe...">
                                <button type="submit" class="bg-blue-600 text-white p-2 rounded-full"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg></button>
                            </div>
                        </form>
                        ...
                        <div class="overflow-y-auto p-2 space-y-2 flex-1">
                            @if(strlen($searchQuery) > 2)
                                <div class="mb-3 border-b border-gray-700 pb-2">
                                    <p class="text-[10px] text-blue-400 uppercase font-bold mb-1">Resultados</p>
                                    @foreach($searchResults as $result)
                                        <div wire:key="search-{{ $result->id }}" class="flex justify-between items-center bg-gray-800/50 p-2 rounded mb-1">
                                            <span class="text-gray-200 text-xs truncate w-24">{{ $result->name }}</span>
                                            @if($result->friend_status === 'none')
                                                <button wire:click="addFriend({{ $result->id }})" class="text-[10px] bg-blue-600 text-white px-2 py-1 rounded">+ Agregar</button>
                                            @elseif($result->friend_status === 'received')
                                                <button wire:click="acceptFriend({{ $result->id }})" class="text-[10px] bg-green-600 text-white px-2 py-1 rounded">Aceptar</button>
                                            @else
                            ...
                            <p class="text-[10px] text-gray-500 uppercase font-bold ml-1">Conectados</p>
                            @forelse($friends as $friend)
                                <div wire:key="friend-{{ $friend->id }}" wire:click="openChat({{ $friend->id }})" 
                            ...
                            @empty
                                <p class="text-xs text-gray-500 text-center py-6">No hay amigos conectados.</p>
                            @endforelse
                        </div>
                    @endif
                ...
                @else
                    ...
                    <div class="flex-1 overflow-y-auto p-4">
                        @if($myParty)
                            <div class="space-y-4">
                                <div class="bg-blue-600/10 border border-blue-500/30 rounded p-4 text-center">
                                    <p class="text-[9px] text-blue-400 uppercase font-black tracking-tighter">Código de Grupo</p>
                                    <p class="text-3xl font-mono font-bold text-white tracking-widest">{{ $myParty->invite_code }}</p>
                                </div>

                                <div class="space-y-2">
                                    <p class="text-[10px] text-gray-500 uppercase font-bold">Miembros ({{ $myParty->members->count() }}/5)</p>
                                    @foreach($myParty->members as $member)
                                        <div class="flex items-center justify-between bg-gray-800/50 p-2 rounded border border-gray-700/50">
                                            <div class="flex items-center gap-2">
                                                <img src="{{ $member->profile_photo_url }}" class="w-6 h-6 rounded-full">
                        ...
                        @else
                            <div class="flex flex-col items-center justify-center h-full space-y-4 px-2">
                                ...
                                <div class="w-full space-y-2">
                                    <input type="text" wire:model="inviteCodeInput" placeholder="EJ: XJ82NS" class="w-full bg-gray-800 border-none rounded text-center font-mono text-white text-sm focus:ring-1 focus:ring-blue-500 uppercase">
                                    ...
                                    @if (session()->has('party_error'))
                                        <p class="text-[10px] text-red-500 text-center">{{ session('party_error') }}</p>
                                    @endif
                                </div>
                            </div>
                        @endif
