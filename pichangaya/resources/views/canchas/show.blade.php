                ...
                <div class="lg:col-span-2 space-y-8">
                        ...
                        @if($totalPhotos > 0)
                            ...
                            @if($totalPhotos > 1)
                                ...
                                <div class="absolute bottom-4 left-0 right-0 z-10 flex justify-center gap-2">
                                    <template x-for="(photo, index) in photos" :key="index">
                                        <button @click.stop="active = index" class="w-2.5 h-2.5 rounded-full transition-all duration-300 shadow-sm border border-white/50" :class="active === index ? 'bg-white w-8' : 'bg-white/40 hover:bg-white/80'"></button>
                                    </template>
                                </div>
                            @endif
                            ...
                            <template x-teleport="body">
                                <div x-show="lightboxOpen" x-transition.opacity.duration.300ms class="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md" style="display: none;">
                                    <button @click="closeLightbox()" class="absolute top-4 right-4 text-white/70 hover:text-white p-2 z-50 transition transform hover:scale-110"><svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                                    <div class="relative w-full h-full flex items-center justify-center p-4" @click.self="closeLightbox()">
                                        @if($totalPhotos > 1)
                                            <button @click.stop="prev()" class="absolute left-4 sm:left-8 text-white/50 hover:text-white transition transform hover:scale-125 p-4 z-50"><svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg></button>
                                        @endif
                                        <img :src="photos[active]" class="max-w-full max-h-full object-contain rounded shadow-2xl transition-transform duration-300">
                                        @if($totalPhotos > 1)
                                            <button @click.stop="next()" class="absolute right-4 sm:right-8 text-white/50 hover:text-white transition transform hover:scale-125 p-4 z-50"><svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg></button>
                                        @endif
                                    </div>
                                </div>
                            </template>
                        @else
                            <div class="flex items-center justify-center h-full bg-gray-200 text-gray-400 text-2xl flex-col gap-2"><span class="text-5xl">🏟️</span><span class="font-bold text-gray-500">Sin Imágenes</span></div>
                        @endif
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
                        <div class="mb-6 border-b border-gray-100 pb-6">
                            ...
                            <p class="text-gray-500 flex items-center text-lg">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-red-500" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" /></svg>
                                {{ $cancha->address }}
                            </p>
                        </div>
                        ...
                        <div class="prose max-w-none text-gray-600 mb-8">
                            <h3 class="text-lg font-bold text-gray-800 mb-3 border-l-4 border-indigo-500 pl-3">Descripción del Lugar</h3>
                            <p class="leading-relaxed">{{ $cancha->description ?? 'El dueño no ha proporcionado una descripción detallada para esta cancha.' }}</p>
                        </div>
                    ...
                    @if($cancha->lat && $cancha->lng)
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 overflow-hidden">
                            <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">📍 Ubicación Exacta</h3>
                            ...
                            <script>
                                function initMapView() {
                                    ...
                                    const map = new google.maps.Map(document.getElementById("map-view"), { center: location, zoom: 16, disableDefaultUI: false, streetViewControl: true });
                                    new google.maps.Marker({ position: location, map: map, title: "{{ $cancha->name }}", animation: google.maps.Animation.DROP });
                                }
                            </script>
                        </div>
                    @endif
