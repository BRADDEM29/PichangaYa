...
<x-app-layout>
    <x-slot name="header">
        ...
        <hgroup class="flex items-center gap-2">
            ...
            {{-- H1: Título Principal Solicitado --}}
            <h1 class="font-bold text-xl text-gray-800 dark:text-gray-200 leading-tight uppercase tracking-wide">
                {{ __('Campeonatos') }}
            </h1>
        </hgroup>
    </x-slot>
    ...
    <main class="py-8 bg-gray-50 dark:bg-gray-900 min-h-screen">
        {{-- SECTION: Contenedor Semántico (Reemplaza al div max-w-7xl) --}}
        <section class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <section aria-labelledby="active-tournaments-heading">
                ...
                @else
                    {{-- SECTION: Grid container (Reemplaza al div grid) --}}
                        ...
                        @foreach($championships as $torneo)
                            ...
                            <article class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 border border-gray-100 dark:border-gray-700 group flex flex-col h-full transform hover:-translate-y-1">
                                ...
                                <header class="h-36 bg-gray-900 relative flex items-center justify-center overflow-hidden">
                                    {{-- SPAN: Overlays decorativos (Reemplazan div absolute) --}}
                                    <span class="absolute inset-0 bg-gradient-to-br from-indigo-900 to-gray-900 opacity-90 block" aria-hidden="true"></span>
                                    ...
                                    <h4 class="text-2xl font-black text-white uppercase tracking-widest z-10 text-center px-6 leading-tight drop-shadow-lg relative">
                                        {{ $torneo->name }}
                                    </h4>
                                </header>
                                
                                {{-- SECTION: Cuerpo del artículo (Reemplaza div p-6) --}}
                                <section class="p-6 flex-1 flex flex-col justify-between">
                                    <section>
                                        {{-- HEADER INTERNO: Estado y Fecha --}}
                                        ...
                                        @if($torneo->prize_description)
                                            {{-- ASIDE: Información del premio --}}
                                            <aside class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg mb-5 flex items-center gap-3 border border-gray-100 dark:border-gray-600">
