            ...
            @foreach($canchas as $cancha)
                <article class="bg-white dark:bg-gray-800 rounded-2xl shadow-md overflow-hidden hover:shadow-2xl transition-all duration-300 border border-gray-100 dark:border-gray-700 flex flex-col h-full group">
                    ...
                    <div class="relative h-56 bg-gray-200 dark:bg-gray-700 overflow-hidden">
                        ...
                        @auth
                            <div class="absolute top-4 left-4 z-20"
                                     ...
                                     toggleFav() {
                                         ...
                                         .then(response => {
                                             if (response.ok) {
                                                 ...
                                                 // Esto avisa a la lista de arriba que debe refrescarse
                                                 if (window.Livewire) {
                                                     window.Livewire.dispatch('refresh-favorites');
                                                 }
                                             }
                                         })
                                 ...
                                 {{-- Escucha cuando la lista de arriba elimina un favorito --}}
                                 @favorite-removed.window="if ($event.detail.id === {{ $cancha->id }}) { isFaved = false; }">
                                
                                <button @click="toggleFav()" 
                                        :disabled="isLoading"
                                        class="bg-white/90 dark:bg-gray-900/90 p-2 rounded-full shadow-md hover:scale-110 active:scale-95 transition-all duration-200 group/btn focus:outline-none">
                    ...
                    <div class="p-5 flex-grow flex flex-col">
                        ...
                        {{-- Resto de tu información (Ubicación, Deportes, etc.) se mantiene igual --}}
                            ...
                            @foreach($cancha->sports as $sport)
                                <span class="bg-green-600 text-white text-[10px] font-bold px-2 py-1 rounded flex items-center gap-1">
                                    <svg class="w-3 h-3 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        {!! config('icons.sports.' . $sport->icon, 'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14l-5-4.87 6.91-1.01L12 2z') !!}
                                    </svg>
                                    {{ $sport->name }}
                                </span>
                            @endforeach
                        ...
                        <div class="mt-auto">
                            <a href="{{ route('canchas.show', $cancha) }}" class="block w-full py-2.5 bg-green-600 text-white text-center rounded-lg font-bold hover:bg-green-700 transition">
                                Ver Disponibilidad
                            </a>
                        </div>
                    </div>
